...
/*
 * APIs adapted from `PropertyUtil.kt` in slack-gradle-plugin.
 */
...
internal fun Provider<String>.mapToLong(): Provider<Long> {
  return map(String::toLong)
}
...
internal fun Project.longProperty(key: String, defaultValue: Long = -1): Long {
  return longProperty(key, provider { defaultValue })
}

internal fun Project.longProperty(key: String, defaultValue: Provider<Long>): Long {
  return safeProperty(key).mapToLong().orElse(defaultValue).get()
}
